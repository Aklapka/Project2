...
var actorChars = {
  "@": Player,
  "o": Hammer, // Hammer will wobble up and down
  "=": Lava, "|": Lava, "v": Lava,
  "L": Loki,
  "g": gem
};
...
Loki.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
	if (!level.obstacleAt(newPos, this.size))
		this.pos = newPos;
	else
...
Hammer.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    // If there aren't any Hammers left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "Hammer";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
